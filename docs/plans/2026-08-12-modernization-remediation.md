...
Add v2 controllers or versioned actions with `ApiVersion("2.0")` using only URL segment selection. Define resource-oriented, lowercase routes and document them in `docs/api/v2-migration.md`; choose and freeze exact routes before release. The baseline v2 routes are:
    ...
    GET  /api/ars/v2/callsigns/{callsign}
    GET  /api/ars/v2/aprs/locations/{callsigns}
    GET  /api/ars/v2/aprs/locations/{callsigns}/coordinates
    GET  /api/ars/v2/aprs/locations/{callsigns}/grids
    POST /api/ars/v2/contacts/enhance-bearing
    GET  /api/ars/v2/configuration/version

For multi-callsign APRS routes, define `{callsigns}` as a comma-separated URL-encoded list, with the maximum count and character pattern stated in OpenAPI. If slash-bearing callsigns need support, require URL encoding or use a repeated query parameter only after documenting one stable choice; do not recreate the v1 catch-all path issue. Use the v2 request DTO to validate `deGrid`, `dxGrid`, and `dxCall` before lookup. The v2 contact response is a new immutable object rather than the input object modified in place. State exact behavior for missing callsign, lookup not found, lookup unavailable, blank grids, and four/six/eight-character locator policy.
...
Example v1-to-v2 migration table to include in `docs/api/v2-migration.md`:
    ...
    /api/ars/v1/Callsign?call={call}              /api/ars/v2/callsigns/{callsign}              Use v2 DTO; handle non-2xx; session fields removed
    /api/ars/v1/Aprs/loc/{id}                     /api/ars/v2/aprs/locations/{callsigns}        Use documented max/encoding and v2 DTO
    /api/ars/v1/Aprs/loc/{id}/coord               /api/ars/v2/aprs/locations/{callsigns}/coordinates Use v2 DTO and non-2xx handling
    /api/ars/v1/Aprs/loc/{id}/grid                /api/ars/v2/aprs/locations/{callsigns}/grids  Expect corrected per-entry grid calculation
