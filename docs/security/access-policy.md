...
Rate limiting is enabled by default through `RateLimiting__Enabled=true`. Set `RateLimiting__Enabled=false` only when the deployment is private and an equivalent network-level control protects QRZ and APRS quotas. The setting takes effect after an application restart; when disabled, the rate-limit middleware and controller policy are not registered.
